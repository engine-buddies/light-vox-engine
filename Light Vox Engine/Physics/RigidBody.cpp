...
int Physics::Rigidbody::CollideBoxBox(const size_t& entityA, const size_t& entityB)
{
	...
	if (best < 3)
	{
		...
		FillPointFaceBoxBox(one, two, toCenter, contacts, componentManager->GetContactsFound(), best, pen);
	...
	else if (best < 6)
	{
		...
		FillPointFaceBoxBox(two, one, toCenter*-1.0f, contacts, componentManager->GetContactsFound(), best - 3, pen);
		componentManager->AddContactsFound();
		return 1;
	}
	else
